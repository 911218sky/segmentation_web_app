...
def main():
    ...
    uploaded_files = st.file_uploader(
        ...
        help="建議每批不超過 200 張圖片，單張不可超過 10MB"
    )
    ...
    if uploaded_files:
        ...
        if process_all:
            ...
            status_text.text(
                f"開始批次處理 {len(images_data)} 張圖片 ({total_batches} 個批次)")
        ...
        if st.session_state.processed_results:
            ...
            # 統計摘要
            successful_results = [
                r for r in st.session_state.processed_results if r['success']]
            failed_results = [
                r for r in st.session_state.processed_results if not r['success']]

            if successful_results:
                col1, col2, col3, col4 = st.columns(4)

                all_mean_lengths = [r['stats']['mean_length']
            ...
            # 計算需要多少行
            successful_with_images = [
                r for r in successful_results if r['result'] is not None]

            if successful_with_images:
                # 每行顯示2張圖片
                cols_per_row = 2
                num_rows = math.ceil(
                    len(successful_with_images) / cols_per_row)
                    ...
                    for col_idx in range(cols_per_row):
                        ...
                        if result_idx < len(successful_with_images):
                            ...
                            with cols[col_idx]:
                                ...
                                with metrics_col1:
                                    st.metric(
                                        "信心度", f"{stats['confidence']:.3f}")
                                    st.metric("測量線數量", stats['num_lines'])
                                with metrics_col2:
                                    st.metric(
                                        "平均長度", f"{stats['mean_length']:.2f} mm")
                                    st.metric(
                                        "標準差", f"{stats['std_length']:.2f} mm")
            ...
            if failed_results:
                ...
                for result in failed_results:
                    st.error(
                        f"**{result['filename']}**: {result['stats'].get('error', '未知錯誤')}")
            ...
            if successful_results:
                ...
                try:
                    ...
                    csv_content = generate_csv_from_results(
                        st.session_state.processed_results)
                ...
                with zipfile.ZipFile(zip_buffer, 'w') as zip_file:
                    ...
                    zip_file.writestr(
                        "configuration_and_analysis_report.txt", "\n".join(config_report))
